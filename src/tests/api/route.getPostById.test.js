...
describe('GET request for post specified with invalid ID)', () => {
  let response;
  let postId = 'invalid';
...
describe('GET request for post specified with valid but non-matching ID)', () => {
  let response;
  let postId = 9999;
  // Prevent axios from throwing an error if HTTP code is not within the 200 range.
  const options = {
...
describe('GET request for post specified with valid and matching ID)', () => {
  let response;
  let postId = 2;
  ...
  it('should return matching post', async () => {
